...
function Moddal({ show, onHide, user, fetchUsers }) {
  ...
  const [isVerified, setIsVerified] = useState(user?.isVerified || false); // Trường trạng thái xác minh
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.put(`auth/updateUser/${user._id}`, {
        ...
        password,
        role,
        isVerified, // Gửi trạng thái xác minh
      });
  ...
  useEffect(() => {
    if (user) {
      ...
      setEmail(user.email);
      setRole(user.role);
      setIsVerified(user.isVerified); // Cập nhật trạng thái xác minh từ user
    }
  }, [user]);
      ...
      <Modal.Body>
        <Form onSubmit={handleSubmit}>
          ...
          <Form.Group controlId="formIsVerified">
            <Form.Label>Trạng Thái Xác Minh</Form.Label>
            <Form.Select
              value={isVerified}
